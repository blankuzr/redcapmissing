...
reduced event/form table with event row-started started/due counts in
the N column, form-incomplete counts nested under each event, and form
rows even when no due records have an exported event row. Repeat form
rows show repeat-instance started/due counts in the N column, while
