...
``` mermaid
flowchart LR
  subgraph EV["same REDCap event"]
    direction TB

    subgraph F1["event:form context 1"]
      direction LR
      subgraph G1A["First gate"]
        direction TB
        A1["event-row-started<br/>non-repeating<br/>on-route"]
        B1["instance-row-started<br/>repeat-instance<br/>on-route"]
      end

      subgraph G2A["Second gate"]
        direction TB
        C1["form-started<br/>on-route"]
        D1["form-complete"]
      end

      subgraph G3A["Third gate"]
        E1["field-complete<br/>on-route"]
      end
    end

    H[" "]

    subgraph F2["event:form context 2"]
      direction LR
      subgraph G1B["First gate"]
        direction TB
        A2["event-row-started<br/>non-repeating<br/>on-route"]
        B2["instance-row-started<br/>repeat-instance<br/>on-route"]
      end

      subgraph G2B["Second gate"]
        direction TB
        C2["form-started<br/>on-route"]
        D2["form-complete"]
      end

      subgraph G3B["Third gate"]
        E2["field-complete<br/>on-route"]
      end
    end
  end

  subgraph G4["Summary"]
    F["event-complete<br/>all forms in event"]
  end

  R["all event<br/>form contexts"]
  ...
  class A1,B1,C1,E1,A2,B2,C2,E2 onRoute;
  class D1,D2,F detour;
  class R rollup;
  class H hidden;
  linkStyle 3,7,10 stroke:#c77900,stroke-width:2px;
```
