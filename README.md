...
reduced event/form table with an `All` summary row, event row-started
started/due counts in the N column, and form-incomplete counts nested
under each event. A form row's `Form Incomplete` numerator is the number
of unique record contexts with failed `event-row-started`,
`instance-row-started`, `form-started`, or `field-complete` rows for
that exact event/form/repeat context. Multiple missing fields in one
record context count once. Its denominator is the exact row-started
assessed N for that context: `event-row-started` for non-repeat
longitudinal rows, `instance-row-started` for repeat rows, and `Total N`
for the display-only non-longitudinal `Single event` row. The `All` row
sums the shown form-row numerators and denominators. Repeat form rows
show repeat-instance started/due counts in the N column, while
