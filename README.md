...
under each event. A form row's `Form Incomplete` numerator is failed
`event-row-started`, failed `instance-row-started`, failed
`form-started`, and failed `form-complete` for that exact
event/form/repeat context. Its denominator is the exact row-started
