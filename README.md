...
started/due counts in the N column, form-incomplete counts nested under
each event, and form rows even when no due records have an exported
event row. The `All` row reports incomplete/assessed form opportunities
across the displayed form rows, using each form row's own
event/form/repeat context. Repeat form rows show repeat-instance
started/due counts in the N column, while non-repeat form rows leave
that cell blank. Projects whose REDCap record ID field has another name
still report the correct N values:
