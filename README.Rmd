...
validation check that ran in this compact example; `get_missing(report)` shows
the failed rows.
...
- `get_missing(report)`: recommended focused missing-row view with nine
  documented columns and optional filtering by canonical validation check
- `report$missing`: full failed rows with native `validation_step` and
  ...
  Entry URL when connection metadata are available; otherwise `NA`
