...
`records` to restrict which record IDs are eligible for assessment on specific
events while leaving omitted events on the default data-derived denominator.
Use `instances` to declare expected repeat instances when REDCap would otherwise
omit nonexistent repeat rows from the export.

`records` is a named list keyed by raw `redcap_event_name`. Non-empty entries
override the record IDs assessed on that event. Empty or omitted events fall
