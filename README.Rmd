...
- `report$missing`: failed validation rows with native `validation_step` and
  `validation_row_id` identifiers; its final `url` column is a raw REDCap Data
  Entry URL when the connection includes the required instance, version, and
  project metadata, plus an event ID for longitudinal projects; otherwise `NA`
...
`validation_level` is `event:form` or `event:form:instance`.
...
numerator is the number of unique record contexts with failed
`event-row-started`, `instance-row-started`, `form-started`, or
`field-complete` rows for that exact event/form/repeat context. Multiple
