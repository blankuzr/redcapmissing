...
In real workflows, `rcon` is not an arbitrary list-like object: it is
ordinarily a `redcapAPI::redcapConnection()` object, and `data` is ordinarily
...
`redcapmissing` extends missingness assessment beyond the REDCap metadata alone
and works around REDCap export behavior to produce informative missingness
reports:
...
`redcap_missing_report()` returns a structured report object that contains a
